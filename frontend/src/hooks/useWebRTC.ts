...
export default function useWebRTC() {
	...
	const [localFrequencyData, setLocalFrequencyData] =
		useState<Uint8Array | null>(null);
	const [remoteFrequencyData, setRemoteFrequencyData] =
		useState<Uint8Array | null>(null);
	...
	useEffect(() => {
		...
		const { audioContext, analyzer, cleanup } =
			setupFrequencyAnalyzer(localStream);
	...
	useEffect(() => {
		...
		const { audioContext, analyzer, cleanup } =
			setupFrequencyAnalyzer(remoteStream);
	...
	const sendSignal = (
		signal: Omit<SignalMessage, 'source'> & { target?: string }
	) => {
